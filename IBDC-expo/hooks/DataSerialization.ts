...
// Data definitions for log file descriptors
enum comm_direction {
    SENDING= "SENDING", // SENDING = 0
    RECEIVING= "RECEIVING", // RECEIVING = 1
}
enum destination {
    DATABASE= "DATABASE",
    PERIPHERAL= "PERIPHERAL",
}
const LOG_FILE: string = "../communicationLog"
...
log_communication(comm_direction["SENDING"], message, message.toString(), destination["DATABASE"]);
...
// Messages will be logged in three places:
    ...
    // 3. When a message is sent to the database or the peripheral (sending)
...
/**
 *
 * @param message_direction comm_direction[SENDING, RECEIVING]
 * @param bytes the proto serialized bytes received over the wire
...
function log_communication(message_direction: comm_direction, bytes: uint8[], text: string, dest: destination): void {
    ...
    console.log(text);
    console.log(dest);
    const log: string = "Logged at " + timestamp + "|" + "proto bytes:" + bytes.toString() + "|" + message_direction + "|" + dest + "\n";
    if (LOG_FILE == undefined) {
        fs.writeFile(LOG_FILE, log, (err) => {
            if (err) {
                console.log(err);
            }
        } )
